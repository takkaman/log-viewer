  ...
  def discover_gr_data(logs, m_vec):
    for k in range(0, len(m_vec)):
      ...
      for i in range(ln + 1, next_ln):
        ...
        gr_iter = ''
        line = logs[i]
        if line == 'Start track assignment':
          gr_phase = 'TA'
          gr_iter = ''
          ...
          if match:
            gr_phase = 'DR'
            gr_iter = match.group(1)
        elif line.startswith('Start GR phase '):
        ...
        if gr_phase != '':
          if prev_gr_phase == gr_phase:
            m.phase += ' ' +  gr_iter
          else:
            if gr_iter != '':
              m.phase += ' ' + gr_phase + ' ' + gr_iter
            else:
              m.phase += ' ' + gr_phase
            prev_gr_phase = gr_phase
    return
