    ...
    for i in range(0, len(logs)):
      ...
      if line.startswith('START_FUNC: ctsInterf::'):
        ...
        if match:
          ...
          # Extract wns/tns/whs/ths results of computeUsefulSkew
          if cts == 'computeUsefulSkew':
            for j in range(i, len(logs) - 2):
              if logs[j].startswith('Final optimized QoR (commit with resolution)'):
                wns = tns = whs = ths = '--'
                match = re.search('\s+WNS\(setup\)=(\S+) TNS\(setup\)=(\S+) .*', logs[j + 1])
                if match:
                  wns = match.group(1)
                  tns = match.group(2)
                match = re.search('\s+WNS\(hold\)=(\S+) TNS\(hold\)=(\S+) .*', logs[j + 2])
                if match:
                  whs = match.group(1)
                  ths = match.group(2)
                if wns != '--' or tns != '--' or whs != '--' or ths != '--':
                  cts += ' {estimated wns %s, tns %s, whs %s, ths %s}' % (wns, tns, whs, ths)
                break
...
class C_regex:
  @staticmethod
  def collect_data(logs, regex):
    m_vec = []
    for i in range(0, len(logs)):
      match = re.search(regex, logs[i])
      if match:
        m = M_regex()
        m.ln = i + 1
        m.info = logs[i]
...
class ICC2LogViewer:
  ...
  def create_argparse(self):
    ...
    parser.add_argument('-regex', help='show all lines that can match the given RegEx expression', type=str)
  ...
  def parse_log_file(self):
    ...
    if self.regex != None:
      m_vec = C_regex.collect_data(self.logs, self.regex)
      for m in m_vec:
        self.info[m.ln] = m
