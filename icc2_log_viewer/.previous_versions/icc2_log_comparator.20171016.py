...
class ICC2LogComparator:
  ...
  def __init__(self):
    ...
    self.setup = False
    self.colored = True
    self.msg_width = 60                     # width of log message
  ...
  def create_argparse(self):
    ...
    group.add_argument('-area',        help='Compare area', action='store_true')
    group.add_argument('-buf',         help='Compare buffer and inverter count', action='store_true')
    group.add_argument('-drc',         help='Compare DRC', action='store_true')
    group.add_argument('-hold',        help='Compare hold timing', action='store_true')
    group.add_argument('-power',       help='Compare leakage and lvth', action='store_true')
    group.add_argument('-runtime',     help='Compare runtime and peak memory', action='store_true')
    group.add_argument('-setup',       help='Compare setup timing (default)', action='store_true')
  ...
  def cprint_header(self):
    ...
    if self.area:
      ...
      header += '%10s %10s %10s' % ('BufCnt', 'InvCnt', 'InstCnt')
    elif self.drc:
  ...
  def get_message(self, m):
    ...
    if m.TAG == 'QOR':
      if self.area:
        ...
        msg += '%10s %10s %10s' % (m.num_buf, m.num_inv, m.num_inst)
      elif self.drc:
        ...
        msg += '%9s %7s' % (m.elapsed, m.peak_mem)
      else:
        msg = m.to_string()
    else:
      msg = m.to_string()
