...
def process(log):
  ...
  ln_delay = log_util.get_next_ln_starts_with(lines, ln_init, 'INFO: check if proc nro_prelude_delay-Wlb-Tb exists')
  if ln_delay != None:
    ln_delay = log_util.get_next_ln_starts_with(lines, ln_delay + 1, 'INFO: check if proc nro_prelude_delay-Wlb-Tb exists')
